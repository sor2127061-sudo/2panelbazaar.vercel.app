...
export default async function AdminOverview() {
  ...
  const totalRevenue = revenue?.reduce((sum: number, o: any) => sum + Number(o.amount_paid), 0) || 0

  return (
    <div className="min-h-screen flex">
      <aside className="w-64 bg-gray-900 border-r border-gray-800 p-6 flex flex-col gap-4">
        <h2 className="text-xl font-bold text-[#0070f3] mb-8">Admin Panel</h2>
        ...
        <div className="mt-auto pt-8"><Link href="/dashboard" className="text-sm text-gray-500 hover:text-gray-300">&larr; Back to App</Link></div>
      </aside>
      <main className="flex-1 p-8">
        <h1 className="text-3xl font-bold mb-8 text-white">Admin Overview</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 bg-gray-900 border border-gray-800 rounded-lg"><h3 className="text-gray-400 mb-2">Total Revenue</h3><p className="text-4xl font-bold text-green-400">৳ {totalRevenue}</p></div>
          <div className="p-6 bg-gray-900 border border-gray-800 rounded-lg"><h3 className="text-gray-400 mb-2">Total Orders</h3><p className="text-4xl font-bold text-white">{ordersCount || 0}</p></div>
          <div className="p-6 bg-gray-900 border border-gray-800 rounded-lg"><h3 className="text-gray-400 mb-2">Total Users</h3><p className="text-4xl font-bold text-white">{usersCount || 0}</p></div>
        </div>
      </main>
    </div>
  )
