...
export default async function ProductPage({ params }: { params: { id: string } }) {
  ...
  async function handleBuy(formData: FormData) {
    ...
    const { data } = await s.rpc('process_purchase', {
      p_user_id: user.id,
        ...
        <form action={handleBuy} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {product.product_packages?.map((pkg: any) => (
              <label key={pkg.id} className="relative flex cursor-pointer rounded-lg border bg-gray-800 border-gray-700 p-4 has-[:checked]:border-[#0070f3] has-[:checked]:ring-1 has-[:checked]:ring-[#0070f3]">
                ...
                <span className="flex flex-col">
                  ...
                  <span className="mt-1 text-sm text-gray-400">৳ {pkg.price}</span>
                </span>
              </label>
            ))}
          </div>
