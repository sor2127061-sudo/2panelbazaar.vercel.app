...
export default async function ProductPackages({ params }: { params: { id: string } }) {
  ...
  async function addPackage(formData: FormData) {
    ...
    await s.from('product_packages').insert({
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Packages: {product?.name}</h1>
        <Link href="/admin/products" className="text-gray-400 hover:text-white">&larr; Back</Link>
      </div>
        ...
        <div>
          ...
          <form action={addPackage} className="bg-gray-900 p-6 rounded-lg border border-gray-800 space-y-4">
            <div>
              ...
              <select name="duration_days" className="w-full p-2 bg-gray-800 rounded border border-gray-700">
            ...
            <div>
              ...
              <input name="price" type="number" step="0.01" min="1" required className="w-full p-2 bg-gray-800 rounded border border-gray-700" />
            </div>
            <div className="flex items-center gap-2">
              <input type="checkbox" name="is_active" id="is_active" defaultChecked className="w-4 h-4" />
              <label htmlFor="is_active">Active</label>
            </div>
            <button className="w-full py-2 bg-primary text-white rounded">Add Package</button>
          </form>
        </div>

        <div>
          <h2 className="text-xl font-bold mb-4">Existing Packages</h2>
          <div className="space-y-4">
            {packages?.map(pkg => (
              <div key={pkg.id} className="p-4 bg-gray-900 border border-gray-800 rounded-lg flex justify-between items-center">
                <div>
                  <div className="font-bold">{pkg.duration_days} Days</div>
                  <div className="text-gray-400">৳ {pkg.price}</div>
                </div>
                <div>
                  <span className={pkg.is_active ? 'text-green-400 text-sm' : 'text-gray-500 text-sm'}>
