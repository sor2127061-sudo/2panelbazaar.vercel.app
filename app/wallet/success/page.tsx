  ...
  if (isValid && searchParams.merchant_order_id) {
    ...
    if (txn?.user_id) {
      await supabase.rpc('credit_wallet', {
        ...
        p_amount: Number(searchParams.amount),
  ...
  return (
    <div className="max-w-md mx-auto p-6 mt-12 text-center">
      <div className={`p-8 rounded-lg border ${isValid ? 'bg-green-900/20 border-green-900' : 'bg-red-900/20 border-red-900'}`}>
        <h1 className="text-2xl font-bold mb-4 text-white">{isValid ? 'Payment Successful' : 'Payment Verification Failed'}</h1>
        <p className="text-gray-400 mb-6">{isValid ? `৳${searchParams.amount} added to wallet.` : 'Signature invalid. Contact support if money was deducted.'}</p>
